...
// uniform sampler2D myTexture; // set from JS
// uniform float uTime;
...
//     float noisy = fbm(p); // Scale vUv for visible noise
//     noisy = noisy * 0.5; // shift from -1..1 → 0..1

//     vec4 p1 = vec4(vposition * 1., uTime * 0.07);
//     float spots = max(fbm(p1), 1.);

//     gl_FragColor = vec4(vec3(noisy), 1.); // Grayscale noise, opaque
//     gl_FragColor *= mix(1., spots, 0.7);

//     // gl_FragColor = vec4(0.26, 0.16, 0.87, 1.0);
// }
...
void main() {
    ...
    vec3 color1 = vec3(1.0, 0.95, 0.0); // Blue for low intensity
    vec3 color2 = vec3(0.89, 0.68, 0.21); // Purple for medium intensity
